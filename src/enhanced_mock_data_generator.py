"""
Enhanced mock data generator for testing WebSocket server with React frontend
"""
...
from src.utils.enhanced_websocket_server import EnhancedWebSocketServer
...
async def generate_and_send_data(server: EnhancedWebSocketServer, technique: str, delay: float = 0.1):
    ...
    for i in range(len(x_data)):
        await server.broadcast_data_point(
            technique=mock_data.technique_type,
            x=x_data[i],
            y=y_data[i]
        )
...
async def run_mock_server(args):
    ...
    try:
        ...
        if args.react_frontend:
            # Use React build directory if it exists
            react_build_dir = os.path.join(os.path.dirname(__file__), 'web-react', 'build')
            if os.path.exists(react_build_dir):
                static_dir = react_build_dir
                logger.info(f"Using React build directory: {static_dir}")
            else:
                logger.warning("React build directory not found. Please run 'npm run build' in src/web-react directory.")
                static_dir = os.path.join(os.path.dirname(__file__), 'web')
        else:
            # Use original web directory
            static_dir = os.path.join(os.path.dirname(__file__), 'web')
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Enhanced mock data generator for WebSocket visualization")
    ...
    parser.add_argument("--react-frontend", action="store_true",
                        help="Use React frontend instead of original HTML/JS frontend")
